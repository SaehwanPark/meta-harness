...
RUNTIME_PROFILE_GLOBS = {
  "codex": ".codex/agents/*.toml",
  "antigravity": ".antigravity/agents/*.md",
  "cursor": ".cursor/agents/*.md",
}
GENERATED_PROFILE_MARKERS = ("# generated by meta harness", "<!-- generated by meta harness")
...
def _profile_paths(root: Path) -> tuple[str, ...]:
  paths: list[Path] = []
  for pattern in RUNTIME_PROFILE_GLOBS.values():
    paths.extend(path for path in root.glob(pattern) if path.is_file())
  return _relative_paths(root, paths)


def _stale_profiles(root: Path, profiles: tuple[str, ...]) -> tuple[str, ...]:
  if (root / ".agents" / "skills" / "harness" / "SKILL.md").is_file():
    return ()
  stale: list[str] = []
  for relative in profiles:
    path = root / relative
    try:
      text = path.read_text(encoding="utf-8", errors="ignore")[:512].casefold()
    except OSError:
      continue
    if any(marker in text for marker in GENERATED_PROFILE_MARKERS):
...
def audit_target(target: str | Path) -> AuditReport:
  ...
  profiles = _profile_paths(root)
  runtimes = _detect_runtimes(root)
  stale = _stale_artifacts(root)
  stale_profiles = _stale_profiles(root, profiles)
  ...
  if stale_profiles:
    risks.append("generated native profiles have no canonical portable skill to execute")
    recommendations.append("restore the portable skill or remove stale generated profiles explicitly")
  if not skills:
  ...
  return AuditReport(
    ...
    operation_classification=_classify(
      skills, roles, runtimes, stale, duplicate_roles, duplicate_skills, stale_profiles
    ),
