...
def run_install(*args: str, env: dict[str, str] | None = None, expect_success: bool = True) -> subprocess.CompletedProcess[str]:
    ...
    if expect_success and result.returncode != 0:
        raise AssertionError(f"Installer failed: {' '.join(command)}\n{result.stderr}\n{result.stdout}")
...
def assert_standard_install(project_root: Path) -> None:
    ...
    assert_true(not shared_skill.is_symlink(), "Expected copy mode to create a standalone SKILL.md")
    assert_true(not (project_root / "AGENTS.md").exists(), "Installer should not create AGENTS.md in the target")
    assert_true(not (project_root / "README.md").exists(), "Installer should not create README.md in the target")
    assert_true(not (project_root / "docs").exists(), "Installer should not create docs/ in the target")
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="meta-harness-install-") as tmp:
        ...
        run_install("--scope", "project", "--target", str(project_standard), "--layout", "standard")
        ...
        assert_true(shared_user_skill.exists(), f"Missing user-level install: {shared_user_skill}")
        assert_true(not shared_user_skill.is_symlink(), "Expected copy mode for user install")

        project_forge = tmp_root / "project-forge"
        project_forge.mkdir()
        run_install("--scope", "project", "--target", str(project_forge), "--layout", "forgecode")
        ...
        project_droid.mkdir()
        run_install("--scope", "project", "--target", str(project_droid), "--layout", "droid")
